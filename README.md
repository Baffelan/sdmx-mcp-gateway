...
- **No channel confirmed a declared set**: `total: 0`, `values: []`, and notes explaining which channel could not answer and why that is not evidence the attribute is missing. The declared set is unknown when the MSD channel answered `too_broad`, `inconclusive` or `unsupported` (whether or not the DSD fallback separately resolved), or when the MSD channel answered `empty` and the DSD fallback did not resolve to `empty`. A DSD-attribute channel answer on its own is not included, since that fallback cannot confirm a declared set; only the MSD channel answering can do that. Like the declared-but-empty case, the notes here do not start with `"Error:"`.
