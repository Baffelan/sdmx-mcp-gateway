...
from tools.sdmx_tools import list_dataflows, get_dataflow_structure
...
class TestLiveSDMXAPI:
    ...
    async def test_spc_dataflow_discovery(self, spc_client):
        ...
        try:
            ...
            assert "data_url_template" in df
            assert "metadata_url" in df
    ...
    async def test_spc_specific_dataflow(self, spc_client):
        ...
        try:
            ...
            structure = await spc_client.get_datastructure("DF_CPI")
            
            if structure and not structure.get("error"):
                assert structure["dataflow_id"] == "DF_CPI"
                assert structure["agency"] == "SPC"
                assert structure["has_structure"] is True
            else:
                # If DF_CPI doesn't exist, just ensure we get proper error handling
                assert "error" in structure or structure is None
    ...
    async def test_api_error_handling(self):
        ...
        client = SDMXClient(base_url="https://stats-sdmx-disseminate.pacificdata.org/rest")
        
        try:
            # Try to get a non-existent dataflow
            structure = await client.get_datastructure("NONEXISTENT_DATAFLOW_123")
            
            # Should handle error gracefully
            assert structure is not None
            if "error" not in structure:
                # Some APIs might return empty structure instead of error
                assert structure.get("has_structure") is False or len(str(structure.get("raw_size_bytes", 0))) > 0
    ...
    async def test_multiple_agencies(self):
        ...
        for agency_id, base_url in agencies_to_test:
            client = SDMXClient(base_url=base_url, agency_id=agency_id)
